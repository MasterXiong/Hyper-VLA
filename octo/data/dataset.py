    ...
    *,
    ...
    if skip_unlabeled:
        ...
        dataset = dataset.filter(
            lambda x: tf.math.reduce_all(tf.strings.length(x["task"]["language_instruction"]) != 0)
        )
...
def apply_frame_transforms(
    ...
    *,
    ...
    def apply_obs_transform(fn: Callable[[dict], dict], frame: dict) -> dict:
        ...
        if "initial_state" in frame:
            frame["initial_state"] = dl.vmap(fn)(frame["initial_state"])
        return frame

    if image_aug_style == "rtx":
        dataset = dataset.frame_map(
            partial(
                apply_obs_transform,
                partial(
                    obs_transforms.decode_and_crop_and_resize,
    ...
    if train and apply_image_augmentation:
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    add_initial_image: bool = False,
    random_initial_image: bool = False,
    initial_image_range: int = 0,
    num_parallel_reads: int = tf.data.AUTOTUNE,
    num_parallel_calls: int = tf.data.AUTOTUNE,
    filter_single_task: Optional[str] = None,
    ...
    def restructure(traj):
        ...
        if add_initial_image:
            if random_initial_image:
                traj_len = tf.shape(traj["action"])[0]
                rand = tf.random.uniform([traj_len])
                low = tf.zeros(traj_len)
                high = tf.cast(tf.range(traj_len), tf.float32)
                if initial_image_range > 0:
                    high = tf.minimum(high, initial_image_range)
                start_idxs = tf.cast(rand * (high - low) + low, tf.int32)
                # sometimes there are floating-point errors that cause an out-of-bounds
                start_idxs = tf.minimum(start_idxs, traj_len - 1)
                traj["initial_state"] = {
                    "image_primary": tf.gather(traj["observation"]["image_primary"], start_idxs)[:, None],
                }
            else:
                initial_image = tf.gather(traj['observation']['image_primary'], 0)
                traj["initial_state"] = {
                    "image_primary": tf.repeat(initial_image[None, None], tf.shape(traj['action'])[0], axis=0),
                }
    ...
    # builder will fail when the name is registered in tfds datasets
    # get around it by manually specifying the data directory
    # builder = tfds.builder(name, data_dir=data_dir)
    version = os.listdir(os.path.join(data_dir, name))[0]
    builder = tfds.builder_from_directory(os.path.join(data_dir, name, version))
    ...
    # filter single-task demos
    def filter_task(traj):

        def is_pick_object_task(instruction):
            start_with_pick = tf.strings.regex_full_match(instruction, r"^pick.*")
            contain_from = tf.strings.regex_full_match(instruction, r".*from.*")
            return tf.math.logical_and(start_with_pick, tf.math.logical_not(contain_from))[0]

        def is_move_near_task(instruction):
            return tf.strings.regex_full_match(instruction, r"^move.*")[0]

        def is_close_drawer_task(instruction):
            return tf.strings.regex_full_match(instruction, r"^close (top|middle|bottom) drawer$")[0]

        # instruction = tf.strings.unicode_decode(traj["observation"]["natural_language_instruction"], "UTF-8")
        instruction = traj["observation"]["natural_language_instruction"]
        if filter_single_task == "simpler_google_robot":
            return tf.math.reduce_any([is_pick_object_task(instruction), is_move_near_task(instruction), is_close_drawer_task(instruction)])
        elif filter_single_task == "pick object":
            return is_pick_object_task(instruction)
        elif filter_single_task == "move near":
            return is_move_near_task(instruction)
        elif filter_single_task == "close drawer":
            return is_close_drawer_task(instruction)
        else:
            return tf.math.reduce_all(instruction == filter_single_task)

    if filter_single_task is not None:
        dataset = dataset.filter(filter_task)
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs in dataset_kwargs_list:
        _, dataset_statistics = make_dataset_from_rlds(**dataset_kwargs, train=train, random_initial_image=random_initial_image, initial_image_range=initial_image_range)
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset, _ = make_dataset_from_rlds(
            **dataset_kwargs,
            ...
            random_initial_image=random_initial_image,
            initial_image_range=initial_image_range,
        )
